...
static void _save_log_to_bmp(char *fname) {
  ...
  // Reconstruct FB for bottom-top, landscape bmp. Rotation: 656x1280 ->
  // 1280x656.
...
static void _save_fb_to_bmp() {
  ...
  s_printf(fname, "%04d%02d%02d_%02d%02d%02d", time.year, time.month, time.day,
...
static void _disp_fb_flush(int32_t x1, int32_t y1, int32_t x2, int32_t y2,
                           const lv_color_t *color_p) {
  // Draw to intermediate non-rotated framebuffer.
  gfx_set_rect_pitch((u32 *)NYX_FB2_ADDRESS, (u32 *)color_p, 1280, x1, y1, x2,
                     y2);
...
static bool _fts_touch_read(lv_indev_data_t *data) {
  ...
  if (console_enabled) {
    ...
    gfx_con.fntsz = 8;
    gfx_printf("x: %4d, y: %4d | z: %3d | ", touchpad.x, touchpad.y,
               touchpad.z);
    gfx_printf("0: %02X, 1: %02X, 2: %02X, ", touchpad.raw[0], touchpad.raw[1],
               touchpad.raw[2]);
    gfx_printf("3: %02X, 4: %02X, 5: %02X, 6: %02X", touchpad.raw[3],
               touchpad.raw[4], touchpad.raw[5], touchpad.raw[6]);
...
static bool _jc_virt_mouse_read(lv_indev_data_t *data) {
  ...
  if (jc_drv_ctx.calibration_step != JC_CAL_MAX_STEPS) {
    if (n_cfg.jc_force_right) {
      if (jc_pad->conn_r && jc_pad->rstick_x > 0x400 &&
          jc_pad->rstick_y > 0x400 && jc_pad->rstick_x < 0xC00 &&
          jc_pad->rstick_y < 0xC00) {
    ...
    } else if (jc_pad->conn_l && jc_pad->lstick_x > 0x400 &&
               jc_pad->lstick_y > 0x400 && jc_pad->lstick_x < 0xC00 &&
               jc_pad->lstick_y < 0xC00) {
      jc_drv_ctx.calibration_step++;
  ...
  console:
    ...
    gfx_con.fntsz = 8;
    gfx_printf(
        "x: %4X, y: %4X | rx: %4X, ry: %4X | b: %06X | c: %d (%d), %d (%d)",
  ...
  if (!n_cfg.jc_force_right) {
    // Left stick X.
    if (jc_pad->lstick_x <= jc_drv_ctx.cx_max &&
        jc_pad->lstick_x >= jc_drv_ctx.cx_min)
      jc_drv_ctx.pos_x += 0;
    ...
    // Left stick Y.
    if (jc_pad->lstick_y <= jc_drv_ctx.cy_max &&
        jc_pad->lstick_y >= jc_drv_ctx.cy_min)
      jc_drv_ctx.pos_y += 0;
    ...
    // Right stick X.
    if (jc_pad->rstick_x <= jc_drv_ctx.cx_max &&
        jc_pad->rstick_x >= jc_drv_ctx.cx_min)
      jc_drv_ctx.pos_x += 0;
    ...
    // Right stick Y.
    if (jc_pad->rstick_y <= jc_drv_ctx.cy_max &&
        jc_pad->rstick_y >= jc_drv_ctx.cy_min)
      jc_drv_ctx.pos_y += 0;
  ...
  // Auto hide cursor.
  if (jc_drv_ctx.pos_x != jc_drv_ctx.pos_last_x ||
      jc_drv_ctx.pos_y != jc_drv_ctx.pos_last_y) {
  ...
  if (jc_pad->b && close_btn) {
    lv_action_t close_btn_action =
        lv_btn_get_action(close_btn, LV_BTN_ACTION_CLICK);
...
void manual_system_maintenance(bool refresh) {
  for (u32 task_idx = 0;
       task_idx < (sizeof(system_maintenance_tasks_t) / sizeof(lv_task_t *));
       task_idx++) {
...
lv_img_dsc_t *bmp_to_lvimg_obj(const char *path) {
  ...
  bmpData.size = bitmap[2] | bitmap[3] << 8 | bitmap[4] << 16 | bitmap[5] << 24;
  bmpData.offset =
      bitmap[10] | bitmap[11] << 8 | bitmap[12] << 16 | bitmap[13] << 24;
  bmpData.size_x =
      bitmap[18] | bitmap[19] << 8 | bitmap[20] << 16 | bitmap[21] << 24;
  bmpData.size_y =
      bitmap[22] | bitmap[23] << 8 | bitmap[24] << 16 | bitmap[25] << 24;
  // Sanity check.
  if (bitmap[0] == 'B' && bitmap[1] == 'M' &&
    ...
    img_desc->header.cf =
        (bitmap[28] == 32)
            ...
            : LV_IMG_CF_TRUE_COLOR; // Only LV_IMG_CF_TRUE_COLOR_ALPHA is
                                    // actually allowed.
...
lv_obj_t *nyx_create_standard_window(const char *win_title,
  ...
  if (!close_action)
    close_btn =
        lv_win_add_btn(win, NULL, SYMBOL_CLOSE " Close", nyx_win_close_action);
  else
...
static void _check_sd_card_removed(void *params) {
  ...
  if (!do_auto_reload && sd_get_card_removed()) {
    ...
    lv_mbox_set_text(mbox, "\n#FF8000 SD card was removed!#\n\n#96FF00 Nyx "
                           "will reload after inserting it.#\n\nReminder that "
                           "you can use UMS instead of removing it.\n");
    lv_mbox_add_btns(
        mbox, h_cfg.rcm_patched ? mbox_btn_map_rcm_patched : mbox_btn_map,
        _removed_sd_action);
...
static lv_res_t _create_mbox_reboot_ofw() {
  ...
  lv_mbox_set_text(mbox, "#FF8000 Warning#\n\n"
...
static lv_res_t _reboot_action(lv_obj_t *btns, const char *txt) {
  ...
  bool t210_8gb_dramid =
      !h_cfg.t210b01 && dram_id == LPDDR4_ICOSA_8GB_SAMSUNG_K4FBE3D4HM_MGXX;
  // Warn against OFW if T210B01 and dram id has the wrong density and ranks
  // with actual ram.
  bool unmatched_fuses =
      h_cfg.t210b01 &&
      dram_id_adj == LPDDR4X_AULA_8GB_SAMSUNG_K4UBE3D4AA_MGCL &&
      dram_id != dram_id_adj;
...
static lv_res_t _create_mbox_reboot(lv_obj_t *btn) {
  ...
  static const char *mbox_btn_map_autorcm[] = {"\261OFW", "\221RCM",
                                               "\221Cancel", ""};
  static const char *mbox_btn_map_patched[] = {"\221OFW", "\221Normal",
                                               "\221Cancel", ""};
  lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
  lv_mbox_set_recolor_text(mbox, true);
  ...
  bool t210_8gb_dramid =
      !h_cfg.t210b01 &&
      fuse_read_dramid(true) == LPDDR4_ICOSA_8GB_SAMSUNG_K4FBE3D4HM_MGXX;
  ...
  if (h_cfg.rcm_patched && !t210_8gb_dramid)
    ...
    lv_mbox_add_btns(
        mbox, !h_cfg.autorcm_enabled ? mbox_btn_map : mbox_btn_map_autorcm,
        _reboot_action);
...
void nyx_create_onoff_button(lv_theme_t *th, lv_obj_t *parent, lv_obj_t *btn,
                             const char *btn_name, lv_action_t action,
                             bool transparent) {
  ...
  if (transparent) {
    ...
    btn_onoff_pr_hos_style.body.main_color =
        LV_COLOR_HEX(theme_bg_color ? (theme_bg_color + 0x101010)
                                    : 0x2D2D2D); // COLOR_HOS_BG_LIGHT.
  btn_onoff_pr_hos_style.body.grad_color =
      btn_onoff_pr_hos_style.body.main_color;
  ...
  lv_obj_align(label_btnsw, btn, LV_ALIGN_IN_RIGHT_MID, -LV_DPI / 4,
               -LV_DPI / 10);
...
static void _create_text_button(lv_theme_t *th, lv_obj_t *parent, lv_obj_t *btn,
                                const char *btn_name, lv_action_t action) {
...
static void _create_tab_about(lv_theme_t *th, lv_obj_t *parent) {
  ...
  lv_obj_align(lbl_octopus, lbl_credits, LV_ALIGN_OUT_RIGHT_TOP, -LV_DPI / 10,
               0);
  ...
  lv_obj_align(hekate_img, lbl_octopus, LV_ALIGN_OUT_BOTTOM_LEFT, 0,
               LV_DPI * 2 / 3);

  lv_obj_t *ctcaer_img = lv_img_create(parent, NULL);
  lv_img_set_src(ctcaer_img, &ctcaer_logo);
  lv_obj_align(ctcaer_img, lbl_octopus, LV_ALIGN_OUT_BOTTOM_RIGHT, 0,
               LV_DPI * 2 / 3);

  char version[32];
  s_printf(version, "Nyx %s%d.%d.%d%c", NYX_VER_RL ? "v" : "", NYX_VER_MJ,
           NYX_VER_MN, NYX_VER_HF, NYX_VER_RL > 'A' ? NYX_VER_RL : 0);
  lv_obj_t *lbl_ver = lv_label_create(parent, NULL);
  lv_obj_align(lbl_ver, ctcaer_img, LV_ALIGN_OUT_BOTTOM_RIGHT, -LV_DPI / 20,
               LV_DPI / 4);
...
static void _update_status_bar(void *params) {
  ...
  s_printf(label, " " SYMBOL_DOT " %d.%d%% ", (batt_percent >> 8) & 0xFF,
           (batt_percent & 0xFF) / 26);
  ...
  s_printf(label, "#%s%d", batt_curr >= 0 ? "96FF00 +" : "FF3C28 ",
           batt_curr / 1000);

  bool voltage_empty = batt_volt < 3200;
  s_printf(label + strlen(label), " mA# (%s%d mV%s)",
           voltage_empty ? "#FF8000 " : "", batt_volt,
           voltage_empty ? " " SYMBOL_WARNING "#" : "");
...
static lv_res_t _win_launch_close_action(lv_obj_t *btn) {
  ...
  for (u32 i = 0; i < (n_cfg.entries_5_col ? 10 : 8); i++) {
    ...
    if (ext->idx) {
      // This gets latest object, which is the button overlay. So iterate 2
      // times.
...
static lv_obj_t *create_window_launch(const char *win_title) {
  ...
  close_btn = lv_win_add_btn(win, NULL, SYMBOL_CLOSE " Close",
                             _win_launch_close_action);
...
static lv_res_t _create_window_home_launch(lv_obj_t *btn) {
  ...
  const launch_button_pos_t *launch_button_pos =
      n_cfg.entries_5_col ? launch_button_pos10 : launch_button_pos8;
  ...
  lv_win_add_btn(win, NULL, SYMBOL_LIST " Logs #D0D0D0 OFF#",
                 logs_onoff_toggle);
  ...
  lv_obj_set_pos(btn_boot_entry, launch_button_pos[0].btn_x,
                 launch_button_pos[0].btn_y);
  ...
  lv_obj_set_pos(boot_entry_lbl_cont, launch_button_pos[0].lbl_x,
                 launch_button_pos[0].lbl_y);
  ...
  for (u32 btn_idx = 1; btn_idx < (n_cfg.entries_5_col ? 10 : 8); btn_idx++) {
    ...
    lv_obj_set_pos(btn_boot_entry, launch_button_pos[btn_idx].btn_x,
                   launch_button_pos[btn_idx].btn_y);
    ...
    lv_obj_set_pos(boot_entry_lbl_cont, launch_button_pos[btn_idx].lbl_x,
                   launch_button_pos[btn_idx].lbl_y);
  ...
  LIST_FOREACH_ENTRY(ini_sec_t, ini_sec, &ini_sections, link) {
    ...
    if (!icon_path) {
      ...
      if (bmp && strlen(icon_path) > 14 &&
          !memcmp(icon_path + strlen(icon_path) - 14, "_hue_nobox", 10)) {
        img_colorize = true;
        img_noborder = true;
      } else {
        // Check if colorization is enabled.
        if (bmp && strlen(icon_path) > 8 &&
            !memcmp(icon_path + strlen(icon_path) - 8, "_hue", 4))
          img_colorize = true;

        // Check if no border is enabled.
        if (bmp && strlen(icon_path) > 10 &&
            !memcmp(icon_path + strlen(icon_path) - 10, "_nobox", 6))
          img_noborder = true;
      }
    }
    ...
    if (img_noborder) {
      ...
      lv_btn_set_style(launch_ctxt.btn[curr_btn_idx], LV_BTN_STYLE_REL,
                       &btn_home_noborder_rel);
      lv_btn_set_style(launch_ctxt.btn[curr_btn_idx], LV_BTN_STYLE_PR,
                       &btn_home_noborder_rel);
    }
    lv_obj_set_size(btns, btn_width, btn_height);
    lv_btn_set_style(btns, LV_BTN_STYLE_REL,
                     img_noborder ? &btn_home_noborder_rel
                                  : &btn_home_transp_rel);
    ...
    // {
    // 	lv_btn_set_style(launch_ctxt.btn[curr_btn_idx], LV_BTN_STYLE_REL,
    // &btn_transp_rel); 	lv_btn_set_style(launch_ctxt.btn[curr_btn_idx],
    // LV_BTN_STYLE_PR,  &btn_transp_pr); 	lv_btn_set_style(btns,
    // LV_BTN_STYLE_REL, &btn_home_noborder_rel);
    // }
    ...
    if (lv_obj_get_width(launch_ctxt.label[curr_btn_idx]) > max_label_size) {
      lv_label_set_long_mode(launch_ctxt.label[curr_btn_idx],
                             LV_LABEL_LONG_ROLL);
...
ini_parse_failed:
  ...
  if (combined_cfg && (curr_btn_idx < (n_cfg.entries_5_col ? 10 : 8)) &&
      !more_cfg)
    goto ini_parsing;
...
static void _create_tab_home(lv_theme_t *th, lv_obj_t *parent) {
  ...
  lv_label_set_static_text(label_tagline,
  ...
  lv_btn_set_action(btn_launch, LV_BTN_ACTION_CLICK,
                    _create_window_home_launch);
  ...
  lv_label_set_text(label_btn, btn_colored_text);
  lv_btn_set_action(btn_more_cfg, LV_BTN_ACTION_CLICK,
                    _create_window_home_launch);
  ...
  _create_text_button(th, NULL, btn_nyx_options,
                      SYMBOL_SETTINGS " Nyx Settings", NULL);
  // lv_obj_set_width(btn_nyx_options, 256);
  lv_btn_set_action(btn_nyx_options, LV_BTN_ACTION_CLICK,
                    create_win_nyx_options);
  lv_obj_align(btn_nyx_options, NULL, LV_ALIGN_IN_BOTTOM_LEFT, LV_DPI / 4,
               -LV_DPI / 12);
  ...
  _create_text_button(th, NULL, btn_power_off, SYMBOL_POWER " Power Off",
                      _create_mbox_poweroff);
  lv_obj_align(btn_power_off, NULL, LV_ALIGN_IN_BOTTOM_RIGHT, -LV_DPI / 4,
               -LV_DPI / 12);

  _create_text_button(th, NULL, btn_reboot, SYMBOL_REBOOT " Reboot",
                      _create_mbox_reboot);
  lv_obj_align(btn_reboot, btn_power_off, LV_ALIGN_OUT_LEFT_MID, 0, 0);

  _create_text_button(th, NULL, btn_reload, SYMBOL_REFRESH " Reload",
                      _create_mbox_reload);
...
static lv_res_t _save_options_action(lv_obj_t *btn) {
  ...
  if (!res)
    lv_mbox_set_text(mbox, "#FF8000 hekate Configuration#\n\n#96FF00 The "
                           "configuration was saved to sd card!#");
  else
    lv_mbox_set_text(mbox, "#FF8000 hekate Configuration#\n\n#FFDD00 Failed to "
...
static void _create_status_bar(lv_theme_t *th) {
  ...
  lv_label_set_text(lbl_battery, " " SYMBOL_DOT " 00.0% " SYMBOL_BATTERY_1
                                 " #FFDD00 " SYMBOL_CHARGE "#");
  ...
  lv_obj_align(lbl_battery, status_bar.battery, LV_ALIGN_OUT_LEFT_MID,
               -LV_DPI / 25, -1);
  ...
  lv_label_set_text(lbl_time_temp,
                    "00:00 " SYMBOL_DOT " " SYMBOL_TEMPERATURE " 00.0");
  ...
  lv_obj_align(lbl_left, lbl_time_temp, LV_ALIGN_OUT_RIGHT_MID, 0,
               -LV_DPI / 14);
  status_bar.temp_symbol = lbl_left;

  lv_obj_t *lbl_degrees = lv_label_create(status_bar_bg, NULL);
  lv_label_set_text(lbl_degrees, "C");
  lv_obj_align(lbl_degrees, lbl_left, LV_ALIGN_OUT_RIGHT_MID, LV_DPI / 50,
               LV_DPI / 14);
...
static lv_res_t _create_mbox_save_changes_action(lv_obj_t *btns,
                                                 const char *txt) {
...
void nyx_check_ini_changes() {
  if (nyx_options_get_ini_changes_made()) {
    ...
    lv_mbox_set_text(mbox, "#FF8000 Main configuration#\n\n"
...
static lv_res_t _show_hide_save_button(lv_obj_t *tv, uint16_t tab_idx) {
  ...
  {
    lv_btn_set_action(status_bar.mid, LV_BTN_ACTION_CLICK,
                      _save_options_action);
...
static void _nyx_main_menu(lv_theme_t *th) {
  ...
  s_printf(version, "Hekate-Alt %s%d.%d.%d%c%c", rel ? "v" : "",
           nyx_str->version & 0xFF, (nyx_str->version >> 8) & 0xFF,
           (nyx_str->version >> 16) & 0xFF, rel > 'a' ? rel : 0,
  ...
  lv_obj_t *footer = lv_label_create(lv_scr_act(), NULL);
  lv_label_set_text(footer, "Mod by Vampitech");
  ...
  system_tasks.task.dram_periodic_comp =
      lv_task_create(minerva_periodic_training, EMC_PERIODIC_TRAIN_MS,
                     LV_TASK_PRIO_HIGHEST, NULL);
  lv_task_ready(system_tasks.task.dram_periodic_comp);

  system_tasks.task.status_bar =
      lv_task_create(_update_status_bar, 5000, LV_TASK_PRIO_LOW, NULL);
  lv_task_ready(system_tasks.task.status_bar);

  lv_task_create(_check_sd_card_removed, 2000, LV_TASK_PRIO_LOWEST, NULL);

  task_emmc_errors =
      lv_task_create(_nyx_emmc_issues_warning, 2000, LV_TASK_PRIO_LOWEST, NULL);
  ...
  if (nyx_str->cfg & NYX_CFG_UMS) {
    ...
    lv_task_t *task_run_ums =
        lv_task_create(nyx_run_ums, LV_TASK_ONESHOT, LV_TASK_PRIO_LOWEST,
                       (void *)&nyx_str->cfg);
  ...
  if (!n_cfg.timeoffset) {
    lv_task_t *task_run_clock = lv_task_create(
        first_time_clock_edit, LV_TASK_ONESHOT, LV_TASK_PRIO_MID, NULL);
    lv_task_once(task_run_clock);
  }

  if (!n_cfg.bpmp_clock)
    task_bpmp_clock =
        lv_task_create(first_time_bpmp_clock, 10000, LV_TASK_PRIO_LOWEST, NULL);
}
...
void nyx_load_and_run() {
  ...
  // Initialize Joy-Con.
  if (!n_cfg.jc_disable) {
    lv_task_t *task_jc_init_hw =
        lv_task_create(jc_init_hw, LV_TASK_ONESHOT, LV_TASK_PRIO_LOWEST, NULL);
  ...
  lv_theme_t *th =
      lv_theme_hekate_init(n_cfg.theme_bg, n_cfg.theme_color, NULL);
  ...
  if (sd_get_mode() == SD_1BIT_HS25) {
    lv_task_t *task_run_sd_errors =
        lv_task_create(_nyx_sd_card_issues_warning, LV_TASK_ONESHOT,
